...
RUN git clone -b $AVALANCHE_VERSION --single-branch https://github.com/lasthyphen/beacongo.git
...
RUN go mod edit -replace github.com/lasthyphen/coreth=../coreth
RUN go mod download
...
COPY --from=builder /go/src/github.com/lasthyphen/beacongo/build .
