...
import (
	...
	avalanchegoMetrics "github.com/lasthyphen/dijetsnodesgo/api/metrics"

	"github.com/lasthyphen/coreth/consensus/dummy"
	corethConstants "github.com/lasthyphen/coreth/constants"
...
func (vm *VM) onExtraStateChange(block *types.Block, state *state.StateDB) (*big.Int, *big.Int, error) {
	...
	for _, tx := range txs {
		...
		if rules.IsApricotPhase4 {
			contribution, gasUsed, err := tx.BlockFeeContribution(rules.IsApricotPhase5, vm.ctx.DJTXAssetID, block.BaseFee())
			if err != nil {
				return nil, nil, err
			}
