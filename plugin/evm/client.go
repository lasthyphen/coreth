...
func NewClient(uri, chain string) Client {
	return &client{
		requester:      rpc.NewEndpointRequester(fmt.Sprintf("%s/ext/bc/%s/djtx", uri, chain)),
...
func (c *client) IssueTx(ctx context.Context, txBytes []byte) (ids.ID, error) {
	...
	err = c.requester.SendRequest(ctx, "djtx.issueTx", &api.FormattedTx{
		Tx:       txStr,
...
func (c *client) GetAtomicTxStatus(ctx context.Context, txID ids.ID) (Status, error) {
	...
	err := c.requester.SendRequest(ctx, "djtx.getAtomicTxStatus", &api.JSONTxID{
		TxID: txID,
	}, res)
...
func (c *client) GetAtomicTx(ctx context.Context, txID ids.ID) ([]byte, error) {
	res := &api.FormattedTx{}
	err := c.requester.SendRequest(ctx, "djtx.getAtomicTx", &api.GetTxArgs{
		TxID:     txID,
		Encoding: formatting.Hex,
	}, res)
	if err != nil {
...
func (c *client) GetAtomicUTXOs(ctx context.Context, addrs []string, sourceChain string, limit uint32, startAddress, startUTXOID string) ([][]byte, api.Index, error) {
	...
	err := c.requester.SendRequest(ctx, "djtx.getUTXOs", &api.GetUTXOsArgs{
		Addresses:   addrs,
...
func (c *client) ListAddresses(ctx context.Context, user api.UserPass) ([]string, error) {
	...
	err := c.requester.SendRequest(ctx, "djtx.listAddresses", &user, res)
...
func (c *client) ExportKey(ctx context.Context, user api.UserPass, addr string) (*crypto.PrivateKeySECP256K1R, string, error) {
	res := &ExportKeyReply{}
	err := c.requester.SendRequest(ctx, "djtx.exportKey", &ExportKeyArgs{
		UserPass: user,
		Address:  addr,
	}, res)
...
func (c *client) ImportKey(ctx context.Context, user api.UserPass, privateKey *crypto.PrivateKeySECP256K1R) (string, error) {
	res := &api.JSONAddress{}
	err := c.requester.SendRequest(ctx, "djtx.importKey", &ImportKeyArgs{
		UserPass:   user,
...
func (c *client) Import(ctx context.Context, user api.UserPass, to, sourceChain string) (ids.ID, error) {
	res := &api.JSONTxID{}
	err := c.requester.SendRequest(ctx, "djtx.import", &ImportArgs{
		UserPass:    user,
		To:          to,
		SourceChain: sourceChain,
	}, res)
...
func (c *client) Export(
	...
	res := &api.JSONTxID{}
	err := c.requester.SendRequest(ctx, "djtx.export", &ExportArgs{
